...
namespace ProjectManagerAPI.Utility
{
    public class RSAKey
    {
        ...
        public static void GenerateKeyAndSave()
        {
            bool wasAdded = false;
            ...
            t.SetParameters(p);
            wasAdded = keyDataAccess.CreateKey(JObject.FromObject(t));
        }
        ...
        public static RSAParameters GetKeyParameters(string user)
        {
            KeyDataAccess keyDataAccess = new KeyDataAccess();
            string rawKey = "";
            rawKey = keyDataAccess.GetKey(user);
            if(rawKey == "")
            {
            ...
            {
                var keyParams = JsonConvert.DeserializeObject<RSAParametersWithPrivate>(rawKey);
